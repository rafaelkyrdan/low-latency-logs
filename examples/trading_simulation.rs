...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Low latency Log - Trading Simulation Example");
    println!("==========================================");
    ...
    init_low_latency_log_with_config(config)?;

    // Register comprehensive message formats for trading
    register_message_format(
        1001,
        "ORDER_NEW: id={}, symbol={}, side={}, qty={}, price={:.2}".to_string(),
    )?;
    register_message_format(
        1002,
        "ORDER_FILLED: id={}, symbol={}, side={}, qty={}, price={:.2}, slippage={:.4}".to_string(),
    )?;
    register_message_format(
        1003,
        "MARKET_DATA: symbol={}, bid={:.2}, ask={:.2}, bid_size={}, ask_size={}".to_string(),
    )?;
    register_message_format(
        1004,
        "STRATEGY_SIGNAL: symbol={}, signal={}, confidence={:.2}, timestamp={}".to_string(),
    )?;
    register_message_format(
        1005,
        "RISK_CHECK: component={}, level={}, threshold={:.2}, value={:.2}".to_string(),
    )?;
    register_message_format(
        1006,
        "PERF_METRIC: component={}, latency_ns={}, throughput={}, queue_depth={}".to_string(),
    )?;
    ...
    for i in 0..10000 {
        ...
        if i % 25 == 0 {
            ...
            let threshold = 0.8;
            low_latency_info!(1005, "position_risk", risk_level, threshold, risk_level);
            message_count += 1;

            if risk_level > threshold {
                low_latency_warn!(2001, "High risk level detected", risk_level);
                message_count += 1;
            }
        }
